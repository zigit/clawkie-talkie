...
For infer failures, return to [`agent-install-infer.md`](agent-install-infer.md).
...
If daemon logs show a pending scope approval request:
...
4. Ask the user to approve the request so the daemon can connect to the gateway for agent turns.
...
Then rerun the agent-turn check in [`agent-install-verification.md`](agent-install-verification.md).
...
- If no phone is available: run the same `openclaw agent --agent main --session-id <exact-session> --channel last --deliver --json -m <smoke-message>` command from the same OS user and environment shape that the service uses.
